    ...
    >
      ...
      {options && isOpen && (
        <div className="absolute top-full left-0 w-48 bg-white dark:bg-slate-800 text-slate-800 dark:text-white shadow-xl rounded-xl py-2 mt-0 border border-emerald-500/10 z-[60]">
          {options.map((opt, idx) => (
            <a key={idx} href={`#${opt.toLowerCase()}`} className="block px-4 py-2 hover:bg-emerald-50 dark:hover:bg-emerald-900/30 hover:text-emerald-600 transition text-xs font-bold">
              {opt}
            </a>
          ))}
        </div>
...
export default function Home() {
  ...
  const menuConfig = [
    { title: "Nosotros", isDropdown: true, items: ["Misión y Visión", "Junta Directiva", "Historia"] },
    { title: "Eventos", isDropdown: false },
    { title: "Afiliados", isDropdown: true, items: ["Directorio", "Beneficios", "Requisitos"] },
    { title: "Formación", isDropdown: true, items: ["Cursos", "Talleres", "Diplomados"] },
    { title: "Convenios", isDropdown: true, items: ["Institucionales", "Comerciales", "Internacionales"] },
    { title: "Normativas", isDropdown: false },
    { title: "Prensa", isDropdown: true, items: ["Noticias", "Galería", "Comunicados"] },
  ...
  return (
    <div className="min-h-screen bg-[#022c22] text-white font-sans selection:bg-emerald-500/30 scroll-smooth">
      <div className={`${darkMode ? 'dark bg-[#022c22]' : 'bg-slate-50'} min-h-screen transition-colors duration-300 font-sans selection:bg-emerald-500/30 scroll-smooth`}>
      
      {/* --- NAVBAR --- */}
      <nav className={`${darkMode ? 'dark bg-[#011a14]/90  border-white/10' : 'bg-white/90  border-[#011a14]/10'} flex items-center justify-between px-6 py-5 lg:px-20 backdrop-blur-md sticky top-0 z-50 border-b `}>
        <div className="flex items-center gap-3">
          <img src={logo} alt="Logo Cámara" className="h-10 w-auto object-contain" />
          <div className="hidden sm:block leading-tight">
             <p className={`${darkMode ? 'dark:text-white' : 'dark:text-black'} font-bold text-sm tracking-widest uppercase`}>Cámara Inmobiliaria</p>
            <p className={`${darkMode ? 'dark:text-white' : 'dark:text-black'} text-emerald-500 text-[10px] font-bold`}>Estado Bolívar</p>
          </div>
        </div>
        
        {/* Links principales*/}
        <div className="hidden xl:flex gap-6 text-[11px] font-bold uppercase tracking-wider dark:text-gray-300 text-slate-600">
          <a href="#inicio" className="text-emerald-500 py-2">Inicio</a>
        ...
        <div className="flex items-center gap-4">
          ...
          <button onClick={() => setIsSesionModalOpen(true)} className="hidden md:block px-5 py-2 text-emerald-500 text-xs font-bold hover:text-emerald-600 transition">
            Login
          </button>
          <button onClick={() => setIsRegisterModalOpen(true)} className="px-6 py-2 bg-emerald-500 text-white dark:text-[#022c22] rounded-full text-xs font-bold hover:shadow-lg hover:shadow-emerald-500/30 transition-all">
            Registro
          </button>
      ...
      {/* --- HERO SECTION --- */}
      <header id="inicio" className="relative scroll-mt-24 px-6 lg:px-20 py-16 lg:py-24 grid lg:grid-cols-2 gap-12 items-center min-h-[95vh] bg-cover"
      style={{  
        ...
        <div className="space-y-6">
          ...
          <h1 className={`${darkMode ? 'dark:text-white' : 'dark:text-white'} text-5xl lg:text-7xl font-bold leading-[1.1] `}>
            Unidos por el <span className="text-emerald-500 italic">progreso</span> inmobiliario de Bolívar
          </h1>
      ...
      <section id="nosotros" className=" scroll-mt-24 bg-white text-slate-900 rounded-t-[4rem] px-6 lg:px-20 py-20 border-b border-gray-100">
        <div className="max-w-4xl mx-auto text-center space-y-8">
          ...
          <p className="text-lg text-slate-600 leading-relaxed italic">
            Somos una Asociación Civil sin fines de lucro, que se rige por la Cámara Inmobiliaria del Estado Bolivar. Agrupa a instituciones y personas tanto naturales como jurídicas, quienes como actores de la industria inmobiliaria y dentro del marco de una visión compartida para el logro de su misión, contribuyen con su acción e inversión al desarrollo del Sector Inmobiliario Venezolano.  
          </p>
          {/* Grid de Cards  */}
      ...
      {[
        ...
        <div key={i} className="group cursor-pointer">
          ...
          <div className="bg-emerald-600 group-hover:bg-emerald-500 text-white p-5 rounded-2xl flex items-center justify-between transition-colors duration-300 shadow-lg shadow-emerald-600/20">
            <span className="font-bold uppercase tracking-wider text-sm">{card.title}</span>
    ...
    <div className="flex justify-center pt-8">
          ...
          <div className="lg:w-1/2 grid grid-cols-2 gap-4">
             <div className="space-y-4">
                <div className="bg-white p-6 rounded-[2rem] shadow-sm border border-emerald-100 text-center">
                   <div className="text-emerald-600 font-bold text-3xl mb-2">220</div>
                   <p className="text-sm text-slate-500 font-medium uppercase tracking-tighter">Afiliados</p>
                </div>
      ...
      <section id="directiva" className="bg-white px-6 lg:px-20 py-24 scroll-mt-24">
        <div className="max-w-7xl mx-auto space-y-16">
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
            <div className="space-y-4">
              <p className="text-emerald-600 font-black uppercase tracking-[0.3em] text-xs">Nuestro Equipo</p>
      ...
      <section id="convenios" className="bg-white px-6 lg:px-50 py-24 scroll-mt-24">
        <div className="max-w-7xl mx-auto space-y-16">
          ...
          <div className="flex flex-wrap items-center justify-between gap-12 grayscale opacity-70 hover:grayscale-0 transition-all duration-700">
            ...
            <div className="h-12 w-auto group cursor-pointer">
              <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQYIgmOl4EASpo1hjggjQq_xP61myeh_nkr9w&s" alt="UCAB" className="h-full object-contain group-hover:scale-110 transition-transform" />
            </div>

            {/* Logo TotalSalud */}
            <div className="h-10 w-auto group cursor-pointer">
               <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQYIgmOl4EASpo1hjggjQq_xP61myeh_nkr9w&s" alt="Total Salud" className="h-full object-contain group-hover:scale-110 transition-transform" />
            </div>

            {/* Logo Fénix Salud */}
            <div className="h-12 w-auto group cursor-pointer">
              <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQYIgmOl4EASpo1hjggjQq_xP61myeh_nkr9w&s" alt="Fénix Salud" className="h-full object-contain group-hover:scale-110 transition-transform" />
            </div>
