...
function CardImage({
  ...
  return (
    <div className="relative mb-2.5 w-full -mx-4 -mt-4 pt-2">
      ...
      <div
        className={`relative w-full h-48 rounded-[1rem] overflow-hidden border-2 border-white dark:border-[#04432f] shadow-sm flex items-center justify-center mt-1.5 ${isLogo ? 'bg-white p-6' : 'bg-[#022c22]'
          }`}
      >
        {url ? (
          <img
        ...
        <div className="absolute bottom-0 left-0 right-0 bg-transparent py-2 z-10 text-center">
          <span className="text-xs font-black text-black tracking-[0.2em] uppercase">
...
export const AfiliadoCard = ({ afiliado, forceRepMode = false, variant = 'default', highlighted = false }: AfiliadoCardProps) => {
  ...
  return (
    <Link
      to={forceRepMode ? `/miembros/${afiliado.id_afiliado}?mode=rep` : `/miembros/${afiliado.id_afiliado}`}
      className={`relative overflow-hidden bg-white dark:bg-[#04432f] rounded-[1.25rem] p-4 shadow-sm border transition-all duration-500 group hover:-translate-y-1 block h-full ${highlighted
