...
const CONVENIOS_INSTITUCIONALES = [
  {
    id: 1,
    nombre: "Alcaldía de Caroní",
    categoria: "Gubernamental",
    logo: null,
  },
  { id: 2, nombre: "Saren", categoria: "Registros", logo: null },
  { id: 3, nombre: "Fedeindustria Bolívar", categoria: "Gremial", logo: null },
  {
    id: 4,
  ...
  { id: 5, nombre: "Seniat", categoria: "Tributario", logo: null },
  { id: 6, nombre: "Cámara de Comercio", categoria: "Gremial", logo: null },
];
      ...
      >
        <div className="text-center space-y-3 md:space-y-4">
          ...
          <h1 className="text-4xl sm:text-5xl lg:text-7xl font-black tracking-tighter drop-shadow-lg">
            Convenios{" "}
            <span className="text-emerald-400 italic">Institucionales</span>
          </h1>
          <p className="text-emerald-100/80 text-xs md:text-sm tracking-widest uppercase font-medium">
            Sector Público y Organismos Aliados
          </p>
        </div>
      </header>
        ...
        {loading ? (
          ...
          <div className="max-w-7xl mx-auto animate-in fade-in slide-in-from-bottom-8 duration-1000">
            <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-3 sm:gap-6 md:gap-8">
              {CONVENIOS_INSTITUCIONALES.map((item) => (
                <div
                  key={item.id}
                ...
                >
                  ...
                  <div className="flex justify-center md:justify-start mb-4 md:mb-6">
                    <span className="inline-block px-2 py-1 md:px-3 bg-slate-100 text-slate-600 text-[9px] md:text-[10px] font-black uppercase tracking-wider rounded-lg border border-slate-200">
                      {item.categoria}
                    </span>
                  </div>

                  <div className="flex flex-col items-center flex-grow justify-center">
                    <div className="w-14 h-14 sm:w-16 sm:h-16 md:w-24 md:h-24 rounded-2xl bg-slate-50 border-2 border-slate-100 flex items-center justify-center overflow-hidden mb-3 md:mb-5 group-hover:bg-white group-hover:border-emerald-100 transition-all duration-500 shadow-inner">
                      {item.logo ? (
                        <img
                          src={item.logo}
                          alt={item.nombre}
                    ...
                    <h3 className="text-xs sm:text-sm md:text-lg font-bold text-slate-800 tracking-tight text-center group-hover:text-[#022c22] transition-colors leading-tight">
                      {item.nombre}
                    </h3>
                  </div>
                </div>
            ...
            <div className="mt-20 md:mt-32 bg-gradient-to-br from-[#022c22] to-[#011a14] rounded-[2rem] md:rounded-[3rem] p-8 md:p-20 text-center text-white relative overflow-hidden shadow-2xl border border-emerald-900/50">
              ...
              <div className="relative z-10 max-w-2xl mx-auto space-y-6 md:space-y-8">
                <h2 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-black tracking-tighter">
                  ¿Representas a una <br />
                  <span className="text-emerald-400 italic">institución?</span>
                </h2>
                <p className="text-emerald-100/70 text-sm md:text-lg leading-relaxed font-medium max-w-xl mx-auto">
                  Trabajamos de la mano con organismos públicos para fortalecer
                  el desarrollo del sector inmobiliario en nuestra región.
                </p>
                <div className="pt-4 md:pt-6">
                  <button className="bg-emerald-500 text-[#022c22] w-full sm:w-auto px-10 py-4 md:px-14 md:py-5 rounded-full font-black text-xs md:text-sm uppercase tracking-[0.2em] hover:bg-white hover:scale-105 transition-all duration-300 shadow-xl active:scale-95">
